...
void QuadMesh::UpdateMesh(Metaballs * blobList, int blobCount) {
	for (int i = 0; i < this->maxMeshSize + 1; i++)
	{
		for (int j = 0; j < this->maxMeshSize + 1; j++)
		{
			VECTOR3D pos = vertices[i*(maxMeshSize + 1) + j].position;
			vertices[i*(maxMeshSize + 1) + j].position.y = 0; //resets y position to 0
			for (int x = 0; x <= blobCount; x++) { //adds height to a vertex from all of the blobs
				float r = 1 + static_cast <float> (rand()) / static_cast <float> (RAND_MAX) / 50; //randomizes the look of hills
				...
				vertices[i*(maxMeshSize + 1) + j].position.y += (blobList[x].height*r)*exp(-((blobList[x].width/r) * distance));
			}
		}
	}
	this->ComputeNormals();
